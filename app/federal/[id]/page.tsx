...
import { LegislativeAPIBill, BillSponsorship, BillDocumentOrVersion } from "@/lib/types/bill";
import { formatDate } from "@/lib/utils";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { notFound } from "next/navigation";

export default async function FederalBillPage({ params }: { params: { id: string } }) {
          ...
          <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {bill.classification?.map((cls: string) => (
                      <span
                        key={cls}
            ...
            {bill.actions && bill.actions.length > 0 && (
              <div>
                ...
                <div className="space-y-4">
                  {bill.actions?.map((action: NonNullable<LegislativeAPIBill['actions']>[number]) => (
                    <div key={action.id} className="border-l-2 pl-4">
                      ...
                      {action.classification && action.classification.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {action.classification.map((cls: string) => (
                            <span
                              key={cls}
                              className="inline-flex items-center rounded-full bg-secondary/50 px-2 py-0.5 text-xs"
                            >
            ...
            {bill.sponsors && bill.sponsors.length > 0 && (
              <div>
                ...
                <div className="grid gap-2">
                  {bill.sponsors.map((sponsor: BillSponsorship) => (
            ...
            {bill.documents && bill.documents.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {bill.documents.map((doc: BillDocumentOrVersion) => (
                    <div key={doc.id} className="bg-muted/50 rounded-lg p-4">
                      <div className="flex flex-col gap-2">
                        <p className="font-medium">{doc.note || "Document"}</p>
                        {doc.links && doc.links.map((link: { url: string; media_type?: string }, idx: number) => (
                          <a
                            key={idx}
                            href={link.url}
                            target="_blank"
            ...
            {bill.versions && bill.versions.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {bill.versions.map((ver: BillDocumentOrVersion) => (
                    <div key={ver.id} className="bg-muted/50 rounded-lg p-4">
                      <div className="flex flex-col gap-2">
                        <p className="font-medium">{ver.note || "Version"}</p>
                        {ver.links && ver.links.map((link: { url: string; media_type?: string }, idx: number) => (
                          <a
                            key={idx}
                            href={link.url}
                            target="_blank"
            ...
            {bill.votes && bill.votes.length > 0 && (
              <div>
                ...
                <div className="space-y-4">
                  {bill.votes?.map((vote: NonNullable<LegislativeAPIBill['votes']>[number]) => (
                    <div key={vote.id} className="bg-muted/50 rounded-lg p-4">
                      <div className="space-y-2">
                        <p className="font-medium">{vote.motion_text || "Vote"}</p>
                        <div className="grid grid-cols-3 gap-4">
                          {vote.counts.map((count: { option: string; value: number }) => (
            ...
            {bill.comments && bill.comments.length > 0 && (
              <div>
                ...
                <div className="space-y-4">
                  {bill.comments?.map((comment: { id: string; text: string; created_at: string }) => (
            ...
            {bill.annotations && bill.annotations.length > 0 && (
              <div>
                ...
                <div className="space-y-4">
                  {bill.annotations?.map((annotation: { id: string; text: string; created_at: string }) => (
